package com.byron.crypto.network
...
class FirestoreService (private var firebaseFirestore: FirebaseFirestore) {

    fun setDocument (data : Any , collectionName: String, id: String, callback: Callback<Void>){

        firebaseFirestore.collection(collectionName).document(id).set(data)
            .addOnSuccessListener { callback.onSuccess(null) }
            .addOnFailureListener { exception ->  callback.onFailed(exception) } //Se propaga la excepción a través del onFailed
    }
    // el callback para notificar si la acción fue exitosa o no a la operación
        ...
        firebaseFirestore.collection(CRYPTO_COLLECTION_NAME)
            ...
            .addOnSuccessListener { result ->
                for (document in result){
                    val criptoList = result.toObjects(Crypto::class.java)
                    callback?.onSuccess(criptoList)
                    break
                }
            }
